...
/**
 * Created by Lxr on 2016/5/1.
 */
/**
 * Created by Csh on 2016/7/28.
 */
public class ElectricalParameter extends BaseActivity implements XListView.IXListViewListener{
...
//                    getData_list("功率因数", jsobObj_child.getString("PF_a").trim());
                                ...
                                if (Uid_int > 0) {
                                    if (Uid_store != Uid_int) {
                                        ...
                                        Toast.makeText(ElectricalParameter.this,"没有新数据", Toast.LENGTH_LONG).show();
                                    }
                                    //  else if(Uid_store==Uid_int)  { rsltlist.add(0,"没有找到新数椐"); }
                                    Uid_store = Uid_int;
                                }else if(Uid_int < 0) {
                                    Toast.makeText(ElectricalParameter.this,"终端掉线", Toast.LENGTH_LONG).show();
                                }
                                listspadp.notifyDataSetChanged();
                                detail_adapter.notifyDataSetChanged();
                            }else {
                                Toast.makeText(ElectricalParameter.this,"返回数据为空", Toast.LENGTH_LONG).show();
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
